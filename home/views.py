...
def format_response(response):
    """
    Formats the chatbot response to display in points (numbered or bullet).
    - Paragraphs are separated with newlines.
    - Numbered lists are formatted with a new line before each item.
    - Bullet points (if no explicit numbering) are converted into bullet-style format.
    """
    # Step 1: Preserve paragraphs by replacing double newlines with two newlines
    response = re.sub(r'\n{2,}', r'\n\n', response)  # Ensure paragraph breaks are preserved

    # Step 2: Format numbered lists (e.g., "1. ", "2) ")
    response = re.sub(r'(\n|^)(\d+[\.\)])\s', r'\1\2 ', response)

    # Step 3: Convert lines without numbers into bullet points
    lines = response.splitlines()
    formatted_lines = []
    for line in lines:
        line = line.strip()
        if re.match(r'^\d+[\.\)]\s', line):  # Numbered list line
            formatted_lines.append(line)  # Keep it as is
        elif line:  # Non-empty line, treat as a bullet point
...
def chat(request):
    if request.method == "POST":
        ...
        if user_input.lower() in greetings:
            # Respond with a predefined greeting
            response = greetings[user_input.lower()]
        else:
            # Generate response using the LLM
            try:
                formatted_prompt = f"{context}\nUser: {user_input}\nAI:"
                raw_response = model6.invoke(formatted_prompt)  # Assume `model6.invoke` generates raw output
                response = format_response(raw_response)  # Format the response for clarity
